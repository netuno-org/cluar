...
import MemberFormModal from "../../../User/Member/FormModal";

const debounces = {}

const MemberTable = forwardRef(({ userData }, ref) => {
  ...
  const onLoadMembers = () => {
    ...
    _service({
      ...
      fail: (error) => {
        ...
        console.error(error);
        notification.error({
          message: Cluar.plainTranslation("member-table-load-failed")
        })
      }
    })
  }
      ...
      render: (val, record) => (
        <Space size={4}>
          <Button
            icon={<EditOutlined />}
            type="text"
            title={Cluar.plainTranslation("member-table-button-edit")}
            onClick={() => {
              console.log(record);
